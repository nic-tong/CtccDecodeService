...
class LogisticRegression(object):
    ...
    def __init__(self, input, n_in, n_out,W,b):
        ...
        self.W = W #theano.shared(value=numpy.zeros((n_in, n_out),
                   #                              dtype=theano.config.floatX),
                    #            name='W', borrow=True)
        # initialize the baises b as a vector of n_out 0s
        self.b = b #theano.shared(value=numpy.zeros((n_out,),
                   #                              dtype=theano.config.floatX),
                    #           name='b', borrow=True)
...
#    #target to the example with the same index in the input.
    ...
    train_set.append(numpy.ndarray(shape=(train_set_size,28*35), dtype=theano.config.floatX));
    train_set.append(numpy.ndarray(shape=(train_set_size), dtype=int));
    valid_set.append(numpy.ndarray(shape=(valid_set_size,28*35), dtype=theano.config.floatX));
    valid_set.append(numpy.ndarray(shape=(valid_set_size), dtype=int));
    test_set.append(numpy.ndarray(shape=(test_set_size,28*35), dtype=theano.config.floatX));
    test_set.append(numpy.ndarray(shape=(test_set_size), dtype=int));
    predict_set.append(numpy.ndarray(shape=(predict_set_size,28*35), dtype=theano.config.floatX));
    predict_set.append(numpy.ndarray(shape=(predict_set_size), dtype=int));
    #pdb.set_trace()
    #load data from kaggle test set
    with open('D:/newTrainAI2.csv', 'rb') as csvfile:
      datareader = csv.reader(csvfile, delimiter=',')
      index=0;
      for row in datareader:
        if index<train_set_size :
          train_set[1][index] = string.atoi(row[0]);
          for pixel_index in xrange(1,28*35+1) : 
    ...
    #load data from kaggle test set
    with open('D:/newTestAI2.csv', 'rb') as csvfile:
      datareader = csv.reader(csvfile, delimiter=',')
      index=0;
      for row in datareader:
        for pixel_index in xrange(0,28*35) : 
...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    classifier = LogisticRegression(input=x, n_in=28 * 35, n_out=35)
